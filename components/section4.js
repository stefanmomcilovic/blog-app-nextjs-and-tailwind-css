        ...
        <div className="flex gap-5">
            <div className="image flex flex-col justify-start">
                <Link href={`/posts/${id}`} rel="follow">
                    <Image src={img || '/'} width="300" height="250" className="rounded" alt={title || "Image"}/>
                </Link>
            </div>
            <div className="info flex justify-center flex-col">
                ...
                <div className="title">
                    <Link href={`/posts/${id}`} rel="follow"><a className="text-xl font-bold text-gray-800 hover:text-gray-600">{title || "Title"}</a></Link>
                </div>
                {author ? <Author /> : <></>}
            </div>
        </div>
